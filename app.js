(() => {
  ...
  /**
   * Converts Google Calendar's HTML description into clean plain text.
   *
   * Google Calendar may return descriptions containing tags such as:
   * <p>, <br>, <div>, <strong>, and HTML entities.
   *
   * This function:
   * - Removes unsafe or unwanted embedded elements
   * - Converts <br> tags into line breaks
   * - Preserves spacing between paragraphs
   * - Decodes HTML entities
   * - Returns safe plain text for textContent
   */
  ...
  function parseDescription(rawDescription = "") {
    ...
    const metadataPattern =
      /^(CATEGORY|IMAGE|BUTTON|URL)\s*:\s*(.+)$/i;
    ...
    try {
      while (requestUrl) {
        ...
        if (data.nextPageToken) {
          ...
          requestUrl =
            `https://www.googleapis.com/calendar/v3/` +
            `calendars/${calendarId}/events?${nextParams}`;
        } else {
          requestUrl = "";
        }
      }
      ...
      setStatus(
        events.length
          ? ""
          : "No events are scheduled in this date range."
      );
      ...
      const explanation =
        ...
        "Confirm that the calendar is public, " +
        "the Calendar API is enabled, the Calendar ID " +
        "is correct, and the API key allows this " +
        "GitHub Pages address.";

      setStatus(
        `${explanation} Google says: ${error.message}`,
        "error"
      );
  ...
  function renderCategoryFilters() {
    const categories = [
      "All",
      ...Array.from(discoveredCategories).sort()
    ];
  ...
  function formatEventDate(event) {
    ...
    if (event.allDay) {
      ...
      if (end) {
        ...
        const sameDate =
          start.toDateString() ===
          inclusiveEnd.toDateString();

        if (!sameDate) {
          return (
            `${dateFormatter.format(start)} – ` +
            `${dateFormatter.format(inclusiveEnd)}`
          );
        }
      }
    ...
    const sameDay =
      comparisonFormatter.format(start) ===
      comparisonFormatter.format(end);
  ...
  function showEventDialog(event) {
    ...
    document.getElementById(
      "dialog-date"
    ).textContent = formatEventDate(event);
    ...
    const imageEl =
      document.getElementById("dialog-image");
  ...
  function initializeCalendar() {
    if (!window.FullCalendar) {
      setStatus(
        "FullCalendar did not load. Check your " +
          "internet connection or CDN settings.",
        "error"
      );
  ...
  if ("ResizeObserver" in window) {
    const observer =
      new ResizeObserver(requestParentResize);
  ...
  if (!isConfigured()) {
    setStatus(
      "Setup required: open config.js and paste " +
        "your public Google Calendar ID and " +
